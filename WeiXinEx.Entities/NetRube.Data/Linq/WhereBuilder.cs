...
namespace NetRube.Data
{
    ...
    internal class WhereBuilder : ExpressionVisitor
    {
        ...
        protected virtual void ParseLike(MethodCallExpression expression, string type)
        {
            ...
            {
                var column = expression.Object as MemberExpression;
                __sql.AppendFormat("({0} LIKE @{1})", this.GetColumnName(withTableName, Database, column), context.Parameters.Count);
            }
            else
            {
                __sql.AppendFormat("({0} LIKE @{1})", new StrBuilder(Database, expression.Object).ToString(), context.Parameters.Count);
            }
            switch (type)
            {
                case "Contains":
